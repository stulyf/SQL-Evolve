...
以下均为 BIRD `dev` 全量 **1534** 题上的 **执行准确率（EX）**：在官方数据库上执行预测 SQL，与标准答案的结果集比对，与 BIRD 榜单评测协议一致。


| 设置                | 含义                                        | 本仓库输出目录                         |
| ----------------- | ----------------------------------------- | ------------------------------- |
| **基线**            | Selector → Decomposer → Refiner，**不**注入技能 | `outputs/bird_dev/`             |
| **+ Skills（本系统）** | 同上流水线，在提示中加载**由基线错题蒸馏**的技能                | `outputs/bird_dev_with_skills/` |


两次实验均使用 `deepseek-chat` 作为骨干 LLM.


| 难度          | 基线（正确 / 总数）    | 基线准确率      | + Skills（正确 / 总数） | + Skills 准确率 | Δ（百分点）    |
| ----------- | -------------- | ---------- | ----------------- | ------------ | --------- |
...
| **总计**      | **914 / 1534** | **59.58%** | **940 / 1534**    | **61.28%**   | **+1.70** |
...
![SQL-Evolve 架构图](./assets/SQL-Evolve.png)
